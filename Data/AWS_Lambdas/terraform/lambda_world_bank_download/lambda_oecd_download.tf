...
resource "aws_iam_role" "lambda_execution_wb" {
  name = "lambda_execution_wb"
...
resource "aws_iam_role_policy_attachment" "lambda_rds_policy" {
  role       = aws_iam_role.lambda_execution_wb.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonRDSDataFullAccess"
}

resource "aws_iam_policy" "lambda_execution_policy_wb" {
  name        = "LambdaExecutionPolicyWb"
...
resource "aws_iam_role_policy_attachment" "lambda_execution_role_policy_attachment" {
  policy_arn = aws_iam_policy.lambda_execution_policy_wb.arn
  role       = aws_iam_role.lambda_execution_wb.name
}

resource "aws_lambda_function" "executable" {
  function_name = "lambda_world_bank_download"
  image_uri     = "669885634214.dkr.ecr.eu-central-1.amazonaws.com/lambda_world_bank_download:latest"
  package_type  = "Image"
  role          = aws_iam_role.lambda_execution_wb.arn
  memory_size   = 256
  timeout       = 900
  environment {
    variables = {
